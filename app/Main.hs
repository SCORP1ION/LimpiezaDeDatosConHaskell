...
instance Csv.FromNamedRecord Partido where
  parseNamedRecord m = Partido
    <$> m Csv..: BS.pack "fecha_partido"
    <*> m Csv..: BS.pack "equipo_local"
    <*> m Csv..: BS.pack "equipo_visitante"
    <*> m Csv..: BS.pack "goles_local"
    <*> m Csv..: BS.pack "goles_visitante"

instance Csv.ToNamedRecord Partido where
  toNamedRecord p = Csv.namedRecord
    [ BS.pack "fecha_partido"    Csv..= fecha_partido p
    , BS.pack "equipo_local"     Csv..= equipo_local p
    , BS.pack "equipo_visitante" Csv..= equipo_visitante p
    , BS.pack "goles_local"      Csv..= goles_local p
    , BS.pack "goles_visitante"  Csv..= goles_visitante p
    ]
...
imputarGoles prom p =
  p { goles_local = Just (fromMaybe prom (goles_local p)) }
...
contarFechasInvalidas =
  V.length . V.filter (isNothing . fecha_partido)
